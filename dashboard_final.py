...
Run this script to show a live 4-panel dashboard:
  ...
  [E] 2D top-down XY projection of trajectory
...
MP4_PATH  = r"C:\Users\aksha\Downloads\loopclosure-20260515T200632Z-3-001\loopclosure\infra1_loopclosure.mp4"

# Odometry: still read from MCAP
MCAP_PATH  = r"C:\Users\aksha\Downloads\loopclosure-20260515T200632Z-3-001\loopclosure\loopclosure.mcap"
...
FRAME_DELAY_S     = 0.02   # Delay between frame releases (controls replay speed)
...
class State:
    ...
    # Odometry (stored as numpy arrays to avoid repeated conversion)
    positions_raw_np = None       # np.ndarray (N, 3) or None
    positions_corrected_np = None # np.ndarray (N, 3) or None

    # Pipeline control
    frame_version = 0   # incremented by reader on each new data push
    data_loaded  = False
    done         = False
...
def reader_thread():
    ...
    for idx, (frame, t_s, match_count) in enumerate(
        ...
        # Draw ORB keypoints manually — color by size, semi-transparent
        ...
        if kp:
            ...
            # Red (small) → Orange (mid) → Yellow (big)  in BGR
            # t=0: red (0,0,255)  t=0.5: orange (0,107,255)  t=1: yellow (0,215,255)
            overlay = frame_bgr_disp.copy()
            for k in kp:
                x, y = int(k.pt[0]), int(k.pt[1])
                r = max(2, int(k.size / 2))
                t = (k.size - s_min) / s_range
                b_ch = 0
                g_ch = int(215 * t)
                r_ch = 255
                cv2.circle(overlay, (x, y), r, (b_ch, g_ch, r_ch), 1, cv2.LINE_AA)

            KP_OPACITY = 0.7
            cv2.addWeighted(overlay, KP_OPACITY, frame_bgr_disp, 1 - KP_OPACITY, 0, frame_bgr_disp)
        ...
        if not loop_already_fired and idx >= skip:
            ...
            if result != "SCANNING..." and not loop_already_fired:
                ...
                with S.lock:
                    ...
                    S.positions_corrected = corrected
        ...
        with S.lock:
            ...
            S.positions_raw_np     = positions_np[:odom_count].copy()
            if loop_already_fired and corrected is not None:
                S.positions_corrected_np = corrected[:odom_count].copy()
            else:
                S.positions_corrected_np = None
            S.frame_version += 1
            S.data_loaded = True
...
ax_traj   = fig.add_subplot(gs[1, 0], projection="3d")    # C: 3D trajectory
ax_traj2d = fig.add_subplot(gs[1, 1])                     # E: 2D top-down XY
...
ax_traj.set_title("[ C ]  3D TRAJECTORY — DRIFT CORRECTION", color=ACCENT, fontsize=8, pad=6, loc="left")
ax_traj2d.set_title("[ E ]  2D TOP-DOWN — XY PROJECTION", color=ACCENT, fontsize=8, pad=6, loc="left")
...
ax_traj.view_init(elev=22, azim=45)

# ── Axis E: 2D top-down XY ────────────────────────────────────
ax_traj2d.set_xlabel("X (m)", fontsize=8)
ax_traj2d.set_ylabel("Y (m)", fontsize=8)
ax_traj2d.set_aspect("auto")
ax_traj2d.grid(True, alpha=0.3)
ax_traj2d.spines[:].set_color(BORDER)

traj2d_corr_line, = ax_traj2d.plot([], [], color=ACCENT,  linewidth=1.5, alpha=0.9, label="Corrected path")
traj2d_start_pt,  = ax_traj2d.plot([], [], marker='o', color=GREEN, markersize=8, zorder=5, linestyle='None', label="Start")
traj2d_end_pt,    = ax_traj2d.plot([], [], marker='o', color=RED,   markersize=8, zorder=5, linestyle='None', label="End")
ax_traj2d.legend(fontsize=7, facecolor=PANEL_BG, edgecolor=BORDER, labelcolor=TEXT, loc="upper left")
...
def update(frame_num):
    with S.lock:
        ...
        ver = S.frame_version
        if ver == _last_version[0]:
            # Still update flash animation for status panel
            if S.loop_detected or not S.done:
                _loop_flash[0] += 1
                if S.loop_detected:
        ...
        # Snapshot shared state (numpy arrays, no conversion needed)
        ...
        raw_np     = S.positions_raw_np
        corr_np    = S.positions_corrected_np
        done       = S.done

    n_frames = len(t_list)
    n_odom   = len(raw_np) if raw_np is not None else 0
    ...
    if n_odom >= 2:
        ...
        if corr_np is not None and len(corr_np) >= 2:
    ...
    # ── Panel E: 2D top-down XY (corrected path only) ────────
    if corr_np is not None and len(corr_np) >= 2:
        xc, yc = corr_np[:, 0], corr_np[:, 1]
        traj2d_corr_line.set_data(xc, yc)
        traj2d_start_pt.set_data([xc[0]], [yc[0]])
        traj2d_end_pt.set_data([xc[-1]], [yc[-1]])

        pad = 0.5
        ax_traj2d.set_xlim(xc.min() - pad, xc.max() + pad)
        ax_traj2d.set_ylim(yc.min() - pad, yc.max() + pad)
